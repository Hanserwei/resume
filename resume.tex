...
\begin{itemize}[parsep=0.4ex]
  ...
  \item 熟悉全链路工具链：CI/CD、日志与指标监控，能独立完成生产环境部署与稳定性治理。
\end{itemize}
...
\begin{itemize}[parsep=0.5ex]
  ...
  \item \textbf{消息与异步：} RocketMQ，熟悉消息可靠性、幂等、顺序与积压处理。
  \item \textbf{工程与运维：} Linux、Docker、Nginx，GitLab CI/CD；掌握日志采集、指标监控与告警的基本建设。
...
\begin{itemize}
  ...
  \item 在采集链路接入异步双写与 RabbitMQ 补偿，结合 Redis 防重锁与 Canal Binlog 延迟校验修复，保障高并发写入的一致性与容灾。
  \item 负责生产管理系统需求拆解与数据库建模，迭代 20+ 业务接口，交付准时率 100\%，并引入 Redis 缓存与异步落库，将核心接口 P95 耗时从 320ms 降至 120ms。
  \item 搭建日志/指标监控与异常告警（SkyWalking + Prometheus），并参与公司 RAG 知识库与本地大模型服务落地，支撑日均 5k+ 调用，故障定位时间缩短 50\%+。
\end{itemize}
